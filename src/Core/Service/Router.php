...
class Router implements KernelServiceInterface, ServiceProviderInterface
{
    ...
    {
        $route = new Route($controllerName, $methodName);
        $this->routes->set($routeName, $route);
        
        // Автоматическая категоризация при регистрации
    ...
    public function resolve(string $requestUri): ?RouteInterface
    {
        error_log("Resolving route: " . $requestUri);
        ...
        $route = new Route(ErrorController::class, 'error404');
        $this->setCurrentRoute($route);
        return $route;
    }
