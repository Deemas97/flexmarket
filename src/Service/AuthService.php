...
class AuthService implements ServiceInterface
{
    ...
    public function login(string $login, string $password, bool $remember = false): bool
    {
        ...
        $sql = "SELECT id, role_id, f, i, email, password_hash, salt, api_token, status, avatar FROM {$this->userTable} 
    ...
    private function initSession(): void
    {
        if (!$this->sessionStarted && session_status() === PHP_SESSION_NONE) {
            if (!is_null($this->userTable) && !empty($this->userTable) && is_string($this->userTable)) {
                session_name($this->userTable);
            }
