...
class VendorsController extends ControllerRendering
{
    ...
    public function index(): ControllerResponseInterface
    {
        ...
        $companyId = $dbConnection->query("SELECT * FROM vendors AS v WHERE v.id = {$userId}")[0]['company_id'];
        ...
        if (($result === false) || empty($result)) {
            ...
            foreach ($result as $vendor) {
                unset($vendor['password_hash'], $vendor['salt'], $vendor['remember_token']);
                $data['vendors'][] = $vendor;
            }
        }
        ...
        return $this->render('vendors/vendors.html.php', $data);
    }
    ...
    public function vendor(int $id): ControllerResponseInterface
    {
        ...
        $sqlGetEmployee = "SELECT * FROM vendors WHERE id = {$id} LIMIT 1";
        ...
        return $this->render('vendors/admin_vendor_edit.html.php', $data);
    }
