...
class VendorAcceptancesApiController extends ControllerAbstract
{
    ...
    public function apiCreate(): ControllerResponseInterface
    {
        if (!$this->checkMethod('POST')) {
            return $this->initJsonResponse(['error' => 'Invalid request method']);
        }
        ...
        try {
            ...
            $db->commit();
            $this->redirect('/vendor/acceptances');
