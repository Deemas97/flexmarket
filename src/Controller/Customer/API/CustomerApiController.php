...
class CustomerApiController extends ControllerAbstract
{
    ...
    public function apiChangePassword(): ControllerResponseInterface
    {
        ...
        if ($this->auth->changePassword($_POST)) {
            $this->redirect('/customer/profile/edit');
        }
    ...
    public function apiUpdate(): ControllerResponseInterface
    {
        ...
        if (!empty($_FILES['avatar']['name'])) {
            $uploadDir = 'uploads/customers/avatars/';
            if (!is_dir($uploadDir)) {
        ...
        $updateSql = "UPDATE customers SET " . implode(', ', $updateParts) . " WHERE id = {$userId}";
